...
//           // Auto-save chat after first user message
//           if (role === 'user' && state.messages.length === 0) {
//             const chatId = state.currentChatId || `chat-${Date.now()}`;
//             const chatTitle = content.slice(0, 50); // Use first message as title
...
//               messages: updatedMessages,
//               currentChatId: chatId,
//               savedChats: [newChat, ...state.savedChats],
//             };
//           }
          
//           // Update existing chat
//           if (state.currentChatId) {
//             const updatedChats = state.savedChats.map(chat =>
...
//         const chatTitle = firstUserMessage?.content.slice(0, 50) || 'Untitled Chat';
        
//         const existingChatIndex = state.savedChats.findIndex(chat => chat.id === chatId);
        
//         const chatData = {
//           id: chatId,
//           title: chatTitle,
//           messages: state.messages,
//           createdAt: existingChatIndex >= 0 ? state.savedChats[existingChatIndex].createdAt : Date.now(),
//           updatedAt: Date.now(),
//         };
        
//         if (existingChatIndex >= 0) {
//           // Update existing chat
//           const updatedChats = [...state.savedChats];
//           updatedChats[existingChatIndex] = chatData;
...
//           set({
//             currentChatId: chatId,
//             savedChats: [chatData, ...state.savedChats],
//           });
//         }
//       },
...
//           ...(state.currentChatId === chatId ? { messages: [], currentChatId: null } : {}),
//         }));
//       },
...
//       // Get all saved chats sorted by updated date
//       getSortedChats: () => {
//         const state = get();
//         return [...state.savedChats].sort((a, b) => b.updatedAt - a.updatedAt);
//       },
      
//       // Search chats
//       searchChats: (query) => {
//         const state = get();
//         if (!query) return state.savedChats;
        
//         const lowerQuery = query.toLowerCase();
//         return state.savedChats.filter(chat =>
//           chat.title.toLowerCase().includes(lowerQuery) ||
//           chat.messages.some(msg => msg.content.toLowerCase().includes(lowerQuery))
//         );
//       },
